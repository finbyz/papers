...
company = "Rahul Papers India Pvt Ltd"
...
if not frappe.db.exists("Item","TEST_ITEM_1"):
    ...
    item_create.create_new_batch = True
...
packaging_material = frappe.db.get_value("Packaging Materials","Box","name")
...
work_order_create.company=company
...
print(frappe.db.get_value("Warehouse",{"company":company, "warehouse_name":"Work In Progress"},"name"))
...
second_si.update_stock = 1
second_si.due_date = date.today() + timedelta(10)
qty = frappe.db.get_value("Batch",final_item_batch_no,"batch_qty")
second_si.append("items",{
        "item_code":"FINISH_TEST_ITEM",
