...
def get_data(filters):
	...
	query = (
		...
		.select(
			...
			po_item.item_code,
			po_item.qty,
			po_item.total_weight,
			po_item.received_qty,
			(po_item.received_qty*po_item.weight_per_unit).as_("received_total_weight"),
			(po_item.qty - po_item.received_qty).as_("pending_qty"),
			IfNull(pi_item.qty, 0).as_("billed_qty"),
			IfNull(pi_item.total_weight, 0).as_("billed_total_weight"),
...
def get_columns(filters):
	...
	columns.extend(
		[
			...
			{
				"label": _("Received Total weight"),
				"fieldname": "received_total_weight",
				"fieldtype": "Float",
				"width": 120,
			},
			{
			...
			{
				"label": _("Billed Total weight"),
				"fieldname": "billed_total_weight",
				"fieldtype": "Float",
				"width": 80,
			},
			{
