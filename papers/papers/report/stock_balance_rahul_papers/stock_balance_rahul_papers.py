...
def get_columns(filters):
	...
	columns = [
		...
		{
			"label": _("Balance Qty(Weight UOM)"),
			"fieldname": "bal_qty_weight_uom",
			"fieldtype": "Float",
			"width": 100,
			"convertible": "qty",
		},
...
def get_item_warehouse_map(filters, sle):
	...
	for d in sle:
		...
		qty_dict.bal_qty_weight_uom = flt(flt(qty_dict.bal_qty) * flt(frappe.db.get_value("Item",d.item_code,'weight_per_unit') if frappe.db.get_value("Item",d.item_code,'weight_per_unit') else 0))
...
def get_item_details(items, sle, filters):
	...
	res = frappe.db.sql(
		"""
		select
			item.name, item.item_name, item.description, item.item_group, item.brand,item.weight_uom, item.stock_uom %s
		from
			`tabItem` item
			%s
		where
