...
def je_validate(self, method):
	if self.voucher_type == "Journal Entry" and self.bill_no and self.pay_to_recd_from:
			jv = frappe.db.exists("Journal Entry",{"docstatus":1,"bill_no":self.bill_no,"voucher_type":"Journal Entry","pay_to_recd_from":self.pay_to_recd_from,'fiscal':self.fiscal})
			if jv :
